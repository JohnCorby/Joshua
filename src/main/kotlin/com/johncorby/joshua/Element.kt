...
/**
 * an higher-level element of code
 */
interface Element {
    override fun toString(): String
}

interface Declare : Element
interface Statement : Element
interface Expr : Element

data class Program(val declares: List<Declare>) : Element {
    ...
    override fun toString() = "struct $name" +
            ...
            funcs.map {
                it.copy(
                    type = it.type,
                    name = "$name$${it.name}",
                    args = listOf(VarDeclare(Type.PTR, "this")) + it.args,
                    block = it.block
                )
            }.joinToString("")
}
