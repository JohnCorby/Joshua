 ...
 */
inline fun <reified T : Ast> ParseTree.visit() = Visitor.visit(this) as T
inline fun <reified T : Ast> List<ParseTree>.visit() = map { it.visit<T>() }


/**
 * convert [code] to [Ast] using the appropriate context gotten using [contextGetter]
 */
inline fun <reified T : Ast> parse(code: String, contextGetter: (GrammarParser) -> ParseTree): T {
